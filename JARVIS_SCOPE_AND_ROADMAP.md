...
### 1.9 What Part 1 does NOT include, stated plainly
Nothing in Part 1 constitutes a Supervisor, a Verifier, a Recovery Engine, an Intent Lock, an Approval Gate, a full Action Firewall, a Tool Registry, a Model Router, a Connector Fabric, MCP integration, browser/computer-use capability, proactive intelligence, or a Family/Household OS. `superpower` is a fixed sequence of skill calls, not a planner in the Stage C sense. `jarvis` is an intent router, not a mission orchestrator. `missions.py` (§1.6) is a state ledger — nothing decides *when* a `READY` step should run on its own. `planner.py`/`plans.py` (§1.7) validate and version a plan a human/skill proposes — `materialize_plan()` only creates `MissionStep` rows, a still-separate, still-manual act. `workers.py` (§1.8) can now execute one already-claimed step through one already-registered worker when explicitly called — it does not decide which step to run next, does not retry, does not verify outcomes, and cannot reach anything outside `jarvis/jarvis.db`. This is the honest baseline the rest of this document measures against.
