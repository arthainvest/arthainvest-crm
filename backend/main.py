...
def fetch_lead_with_member_name(cursor, lead_id):
    ...
    cursor.execute(
        ...
        LEFT JOIN quotations ON quotations.id = leads.quotation_id
        LEFT JOIN contacts AS linked_contact ON linked_contact.id = leads.contact_id
...
def fetch_contact_with_member_name(cursor, contact_id):
    ...
    cursor.execute(
        ...
        LEFT JOIN calls ON calls.id = contacts.call_id
        WHERE contacts.id = ?
        """,
        (contact_id,)
    )
...
async def link_lead_task(lead_id: int, link: LeadTaskAssign, token: str = Query(None)):
    ...
    with get_db() as conn:
        ...
        cursor.execute(
            "UPDATE leads SET task_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
            (link.task_id, lead_id)
        )
        conn.commit()
...
async def link_lead_quotation(lead_id: int, link: LeadQuotationAssign, token: str = Query(None)):
    ...
    with get_db() as conn:
        ...
        cursor.execute(
            "UPDATE leads SET quotation_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
            (link.quotation_id, lead_id)
        )
        conn.commit()
...
@app.put("/api/leads/{lead_id}/contact", response_model=LeadResponse)
async def link_lead_contact(lead_id: int, link: LeadContactAssign, token: str = Query(None)):
    ...
    with get_db() as conn:
        ...
        cursor.execute(
            "UPDATE leads SET contact_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
            (link.contact_id, lead_id)
        )
        conn.commit()
...
async def link_deal_call(deal_id: int, link: DealCallAssign, token: str = Query(None)):
    ...
    with get_db() as conn:
        ...
        cursor.execute(
            "UPDATE deals SET call_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
            (link.call_id, deal_id)
        )
        conn.commit()
...
async def exotel_status_webhook(request: Request):
    """Exotel POSTs here (form-encoded) as a bridged call progresses and again once it ends.
    No auth header Exotel can be made to send, so this is deliberately unauthenticated, same
    as the WhatsApp webhook's verify step - CustomField (our own calls.id, set when the call
    was placed in _dial_via_exotel) is what finds the right row, not Exotel's own Sid (see
    that function's docstring for why). Silently no-ops on a missing/unrecognized CustomField
    rather than erroring back at Exotel, since retries/replays of partial callbacks are
    expected from their side."""
    form = await request.form()
    call_id = form.get("CustomField")
    if not call_id:
        return {"ok": True}

    status = (form.get("DialCallStatus") or form.get("Status") or "").lower()
    ...
    duration_val = None
    try:
        duration_val = int(form.get("DialCallDuration") or form.get("Duration") or "")
    except ValueError:
        pass
    ...
    if updates:
        ...
        with get_db() as conn:
            cursor = conn.cursor()
            cursor.execute(f"UPDATE calls SET {', '.join(updates)} WHERE id = ?", params)
            conn.commit()
