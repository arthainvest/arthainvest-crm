...
import {
  ...
  getAutomationEnrollments, enrollGroupInAutomation, stopEnrollment,
...
const TRIGGER_LABELS = {
  ...
  new_conversation: 'New conversation (recorded, not yet auto-fired)',
...
export default function Automations() {
  ...
  const handleStopEnrollment = async (enrollmentId) => {
    try {
      await stopEnrollment(token, enrollmentId);
  ...
  return (
    <div className="automations-container">
      <div className="automations-header">
        <div>
          <h1>Automations</h1>
          <p className="automations-subtitle">Drip sequences that send a series of WhatsApp messages over time to a group of contacts/leads.</p>
        </div>
        ...
        {automations.length === 0 ? (
          ...
          <div key={automation.id} className="automation-card">
            ...
            <div className="automation-enroll-row">
              ...
              <button className="btn-primary enroll-btn" onClick={() => handleEnrollGroup(automation)}>Enroll Group</button>
            </div>
          </div>
        ))}
      </div>
              ...
              <div className="modal-body">
                ...
                {form.trigger_type !== 'manual' && (
                  <p className="automation-trigger-hint">
                    Note: this trigger is recorded for reference, but nothing auto-enrolls a conversation yet -
                ...
                <p className="automation-hint">
                  ...
                  required for the first message to someone who hasn't. Template variables aren't supported inside
                  automation steps yet; use a template with no variables.
                </p>
              </div>
      ...
      {showGroupForm && (
        <div className="modal-overlay" onClick={() => setShowGroupForm(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>New Group</h2>
              <button className="btn-close" onClick={() => setShowGroupForm(false)}>×</button>
            </div>
        ...
        <div className="modal-overlay" onClick={() => setShowEnrollments(false)}>
          <div className="modal-content enrollments-modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="btn-close" onClick={() => setShowEnrollments(false)}>×</button>
            </div>
            <div className="modal-body">
                ...
                <div className="enrollments-table-wrap">
                  <table className="enrollments-table">
                    ...
                    <tbody>
                      {enrollments.map((en) => (
                        <tr key={en.id}>
                          <td>{en.contact_name || en.lead_name || en.wa_number}</td>
